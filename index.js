...
function parseBlockLevel(BLOCK_LEVEL) {
 ...
 if (BLOCK_LEVEL === 'CRITICAL') return 4;
...
async function checkUltronHealth(url) {
  ...
  for (let attempt = 0; attempt < ULTRON_HEALTH_BACKOFF_DELAYS.length; attempt++) {
    ...
    console.log(`Health check failed (attempt ${attempt + 1}/${ULTRON_HEALTH_BACKOFF_DELAYS.length}). Retrying in ${ULTRON_HEALTH_BACKOFF_DELAYS[attempt] / 1000}s...`);
...
async function run() {
  ...
  const ultronHealthy = await checkUltronHealth(ULTRON_HEALTH_URL);
